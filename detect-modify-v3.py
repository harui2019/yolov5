...
try:
    ...
    raise ImportError("'detect-modify-v2.py' is not in the directory 'yolov5' which git clone from 'ultralytics/yolov5'")
...
def InitUltralytics(tgtFolder):
    ...
    tgtFolder = Path(tgtFolder).resolve()
    print(tgtFolder)
    if tgtFolder.exists():
        os.chdir(tgtFolder)
...
def RawedCatchClass(weights):
    ...
    if pt:
        ...
        raise ImportError("we can only get class name from '.pt'.,", os.getcwd(), weights)


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='./runs/train/.invoiceRecognize/weights/best.pt', help='best.pt for invoice')
    ...
    parser.add_argument('--char-weights',nargs='+',type=str, default='./runs/train/.invoiceCharacter/weights/best.pt', help='best.pt for characteristic')
...
def main(opt):
    ...
    '''
    {
        ...
        'tfl_int8': False, 'cropname_full': False, 'yolov5_locate': 'test'
    }
    '''
    ## Environment
    ...
    WeightFilePath = Path(all_vars['weights'])
    # raise KeyboardInterrupt(UltralyticsYOLOv5Folder/all_vars['weights'])
    names = RawedCatchClass(Path(UltralyticsYOLOv5Folder) / WeightFilePath)
    # names = RawedCatchClass(WeightFilePath)
    ImagesFolder = increment_path(Path(UltralyticsYOLOv5Folder) / Project / OutputFolderName / '', exist_ok=Increment)
...
##        print('-'*20 + 'start taking note about ' + img + '-'*20) <- if this line can't run successfully, there's path problem with last line
        img = Path(img)
    ...
    for k in AllLabel.keys():
        ...
        for k2 in AllLabel[k]['class'].keys():
            ...
            print(">>> {}: {}, {}".format(k2, cropinfo['subclass_str'], [relX, relY, relW, relH]))
    ...
    print('### Read AllLabel.json from ' + str(save_dir))
    universal_cmd('python split_json.py --path ' + str(save_dir.stem))
    print('-'*30)

    #########################
    # prepare arguments of characteristic detect
    print('### Detect characteristic of train tickets, bus receipts ,and VAT receipts')
    # run_vars['source'] = str(save_dir/Path('crops'))
    # run_vars['weights'] = all_vars['char_weights']
    # run_vars['exist_ok'] = True
    # run_vars['name'] = 'char_exp'
    # run_vars['save_txt'] = True
    # invoice_dir = save_dir

    #########################
    # ReadyDetect
    print("-"*30+"\n ### Run `'detect.py'`")
##    try:
##        print(json.dumps(run_vars,sort_keys=False,indent=4))
##    except:
##        print(run_vars)
    ## Run `detect.py`
    run_vars_2 = run_vars.copy()
    run_vars_2['source'] = str(save_dir/Path('crops'))
    run_vars_2['weights'] = all_vars['char_weights']
    run_vars_2['exist_ok'] = True
    run_vars_2['name'] = 'char_exp'
    run_vars_2['save_txt'] = True
    run_vars_2['save_conf'] = True
    run_vars_2['conf_thres'] = 0.5
    run_vars_2['imgsz'] = [512,512]
    run(**run_vars_2)

    #########################
    # filter certified document
    print('-'*30 + '\n### Filter certified document')
    print(
        'python find_certified_doc.py --path1 ' + str(save_dir.stem) +
        ' --path2 ' + str(Path(run_vars_2['name'])))
    universal_cmd(
        'python find_certified_doc.py --path1 ' + str(save_dir.stem) +
        ' --path2 ' + str(Path(run_vars_2['name'])))
    print("#"*20)
